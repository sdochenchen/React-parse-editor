let equalObjects = require('./equalObjects')
let Id = require('./Id')
...
function queryHash(query) {
  ...
  let where = flattenOrQueries(query.where || {})
  let columns = []
  let values = []
  let i
  ...
  let sections = [columns.join(','), stringify(values)]
...
function matchesKeyConstraints(object, key, constraints) {
  ...
  if (key.indexOf('.') >= 0) {
    ...
    let subObjectKey = keyComponents[0]
    let keyRemainder = keyComponents.slice(1).join('.')
  ...
  // More complex cases
  for (let condition in constraints) {
    ...
    switch (condition) {
      ...
      case '$regex':
        ...
        let exp = new RegExp(expString, constraints.$options || '')
      ...
      case '$nearSphere':
        ...
        let distance = compareTo.radiansTo(object[key])
        let max = constraints.$maxDistance || Infinity
        return distance <= max
      case '$within':
        if (!compareTo || !object[key]) {
          return false
        }
        let southWest = compareTo.$box[0]
        let northEast = compareTo.$box[1]
